# Operations Procedure document template

This is the template for SSDC operations procedure document repositories (`OP` document category) written in LaTeX.
...
```shell
cookiecutter gh:SPHEREx/spherex-templates --directory ssdc-op-latex
```
...
```shell
git remote add origin https://github.com/IPAC-SW/ssdc-op-<...>.git
git branch -M main
...
b. Copy over any content from an external draft document.
...
d. Copy any images from the original document into the new repo.
...
Once you have created the Pull Request, or via the **Actions** tab near the top of the GitHub UI (e.g., https://github.com/IPAC-SW/ssdc-op-008/actions), you should see a summary of the execution of the automatic LaTeX workflow that runs when you push or make a pull request — green: success, yellow: still in progress, red: failed.
If it failed, you should be able to click through to see the logs and see any errors, e.g., from LaTeX.

If the workflow succeeded, you can click through to it (e.g., https://github.com/IPAC-SW/ssdc-op-008/actions/runs/) and you should see on the lower half of the page that there’s an “artifact” list which includes a PDF for the document.  The PDF name is the document handle plus the 7-digit “SHA” identifier for the commit from which it was generated.
...
You can see one here: https://github.com/IPAC-SW/ssdc-op-008/releases.
