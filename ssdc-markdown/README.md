...
```shell
git remote add origin https://github.com/IPAC-SW/ssdc-tn-<...>.git
git branch -M main
...
If all is well, continue working on on your local repository clone to add the actual text and any figure files that are needed.

1. Start by creating a branch for this work:

   ```shell
   git checkout -b initial-text
   ```

2. Update metadata, such as author lists, in the `technote.toml` file. The [Technote package documentation](https://technote.lsst.io/user-guide/technote-toml.html) has more information on the metadata fields.

3. Copy over the content from the original document.
   Add the document's summary to the `abstract` directive — don't create a separate abstract section.
   Add the document's main content into sections below the abstract.
   The "References" section is set up to show citations made with the [sphinxcontrib.bibtex extension](https://sphinxcontrib-bibtex.readthedocs.io/en/latest/usage.html).

   [For information about the Markdown syntax used in this document, see the MyST-Parser documentation](https://myst-parser.readthedocs.io/en/latest/).

4. Copy any images from the original document on Overleaf into the new repo.

5. If you'd like, you can compile the document locally.
...
6. Once ready, push this content to GitHub:
...
The command will include a URL to create a new Pull Request (PR) based on the branch you just pushed.
...
Once you have created the Pull Request, or via the **Actions** tab near the top of the GitHub UI (e.g., https://github.com/IPAC-SW/ssdc-tn-009/actions), you should see a summary of the execution of the automatic LaTeX workflow that runs when you push or make a pull request — green: success, yellow: still in progress, red: failed.
If it failed, you should be able to click through to see the logs and see any errors, e.g., from Sphinx.

If the workflow succeeded, you can view the branch-specific HTML preview at https://spherex-docs.ipac.caltech.edu/{{handle}}/v/initial-text/.
