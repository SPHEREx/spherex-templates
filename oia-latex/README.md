# Operational Interface Agreement document template

This is a template for SPHEREx OIA document repositories (`IF` document category), written in LaTeX.
...
```shell
cookiecutter gh:SPHEREx/spherex-templates --directory oia-latex
```
...
Instead of a document title, you need specify only the name of the partner organization.
You'll be shown the auto-generated title to confirm.
...
If all is well, continue working on on your local repository clone to add the actual text,
as well as any figure files that are needed.
...
For the initial load into GitHub, *we are keeping the version numbers as they last appeared on Overleaf.*
