...
\shortTitle{Requirement {{ cookiecutter.req_doors_id }} Closure Memo}
\ReqDoorsID[https://cae-tracetree-spherex.jpl.nasa.gov/vi{{- cookiecutter.req_doors_id -}}.html]{ {{- cookiecutter.req_doors_id -}} }
\IPACJiraID{ {{- cookiecutter.req_jira_id -}} }
...
% The document history cannot be rendered while keeping the document to a single page.
% Please maintain it, though commented out, so that some record of intent remains in
...
% Multiple VAs may appear in the above table and then in the list of narrative descriptions
% that follows.
