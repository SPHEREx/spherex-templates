# Test Report (wrapper) document template

This is a template for SPHEREx release test report repositories (`TR` document category),
for use when the test report is written in MarkDown and converted to LaTeX with `pandoc`.
This is a temporary measure while we engineer a more automated solution.
...
**Note that this template assumes that the output of `pandoc` will be named `test-report.tex`.**
...
```shell
cookiecutter gh:SPHEREx/spherex-templates --directory test-report-wrapper
```
...
If all is well, continue working on on your local repository clone to add the actual test report LaTeX file, produced by `pandoc`,
which will be used as input to the template LaTeX file `SSDC-TR-nnn.tex`, as well as any figure files that are needed.

a. Start by creating a branch for this work, e.g.,

```shell
git checkout -b tickets/SP-nnn
```
...
git add .
git commit
git push -u origin tickets/SP-nnn
```
